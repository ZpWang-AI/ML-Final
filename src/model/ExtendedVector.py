...
class ExtendedVector(nn.Module):
    def __init__(
        self, 
        data_dim,
        channels,
        ...
        self.extended_vector = nn.Parameter(torch.randn((336,data_dim)), requires_grad=True)
    ...
    def forward(self, inputs):
        ...
        add = self.extended_vector[:y.shape[1], ]
        add = add.unsqueeze(0).repeat((y.shape[0],1,1))
        tgt = torch.concat([tgt, add], dim=1)
